...
void phg::SIFT::buildPyramids(const cv::Mat &imgOrg, std::vector<cv::Mat> &gaussianPyramid, std::vector<cv::Mat> &DoGPyramid) {
    ...
    for (size_t octave = 0; octave < NOCTAVES; ++octave) {
        ...
        for (ptrdiff_t layer = 1; layer < OCTAVE_GAUSSIAN_IMAGES; ++layer) {
            double sigmaCur  = INITIAL_IMG_SIGMA * sqrt(pow(k, layer) * pow(k, layer) - 1);     // sigma12 - сигма до которой мы хотим дойти на текущем слое

            cv::Mat imgLayer = gaussianPyramid[octave * OCTAVE_GAUSSIAN_IMAGES].clone();
            cv::Size automaticKernelSize = cv::Size(0, 0);

            cv::GaussianBlur(imgLayer, imgLayer, automaticKernelSize, sigmaCur, sigmaCur);
            ...
            for (size_t layer = 1; layer + 1 < OCTAVE_DOG_IMAGES; ++layer) {
                ...
                for (ptrdiff_t j = 1; j < cur.rows - 1; ++j) {
                    for (ptrdiff_t i = 1; i + 1 < cur.cols; ++i) {
                        ...
                        // сделать честное уточнение координаты особой точки и итеративное уточнение пикселя
                        ...
                        int dxForC = i + dx - (int)(i + dx);
                        int dyForC = j + dy - (int)(i + dy);

                        dvalue = dxForC * dyForC * DoGs[1].at<float>((int)(i + dy) + 1, (int)(i + dx) + 1) +
                                (1 - dxForC) * dyForC * DoGs[1].at<float>((int)(i + dy) + 1, (int)(i + dx)) +
                                (1 - dyForC) * dxForC * DoGs[1].at<float>((int)(i + dy), (int)(i + dx) + 1) +
                                (1 - dxForC) * (1 - dyForC) * dxForC * DoGs[1].at<float>((int)(i + dy), (int)(i + dx));

#endif
                        float contrast = dvalue;
                        if (abs(contrast) < contrast_threshold / OCTAVE_NLAYERS) // TODO почему порог контрастности должен уменьшаться при увеличении числа слоев в октаве?
                            continue;

                        double Hxx = DoGs[1].at<float>(j, i + 1) - DoGs->at<float>(j, i - 1) - 2 * contrast,
                               Hyy = DoGs[1].at<float>(j + 1, i) - DoGs->at<float>(j - 1, i) - 2 * contrast,
                               Hxy = DoGs[1].at<float>(j + 1, i + 1) + DoGs->at<float>(j - 1, i - 1) -
                                     DoGs[1].at<float>(j + 1, i - 1) + DoGs->at<float>(j - 1, i + 1);

                        if (Hxx * Hyy - Hxy * Hxy / 16 <= 0)
                            continue;

                        if ((Hxx + Hyy) * (Hxx + Hyy) / (Hxx * Hyy - Hxy * Hxy / 16) >= 12.1)
                            continue;
...
bool phg::SIFT::buildLocalOrientationHists(const cv::Mat &img, size_t i, size_t j, size_t radius,
    ...
    for (size_t y = j - radius + 1; y < j + radius; ++y) {
        for (size_t x = i - radius + 1; x < i + radius; ++x) {
            // m(x, y)=(L(x + 1, y) − L(x − 1, y))^2 + (L(x, y + 1) − L(x, y − 1))^2
             double magnitude = std::sqrt((img.at<float>(y, x + 1) - img.at<float>(y, x - 1)) *
    ...
    float filtr_sum[ORIENTATION_NHISTS];
    filtr_sum[0] = sum[ORIENTATION_NHISTS - 1] * 0.15 + sum[0] * 0.7 + sum[1] * 0.15;
    for (size_t bin = 1; bin < ORIENTATION_NHISTS - 1; bin++) {
        filtr_sum[bin] = sum[bin - 1] * 0.15 + sum[bin] * 0.7 + sum[bin + 1] * 0.15;
    }
    filtr_sum[ORIENTATION_NHISTS - 1] = sum[ORIENTATION_NHISTS - 2] * 0.15 + sum[ORIENTATION_NHISTS - 1] * 0.7 + sum[0] * 0.15;
                ...
                for (int smpi = 0; smpi < DESCRIPTOR_SAMPLES_N; ++smpi) { // перебираем столбик очередного замера для текущей гистограммы
                    for (int smpy = 0; smpy < smpW; ++smpy) { // перебираем ряд пикселей текущего замера
                        for (int smpx = 0; smpx < smpW; ++smpx) { // перебираем столбик пикселей текущего замера
                            ...
                            double magnitude = std::sqrt((img.at<float>(y + 1, x) - img.at<float>(y - 1, x)) *
                                                         (img.at<float>(y + 1, x) - img.at<float>(y - 1, x)) +
                                                         (img.at<float>(y, x + 1) - img.at<float>(y, x - 1)) *
                                                         (img.at<float>(y, x + 1) - img.at<float>(y, x - 1)));
