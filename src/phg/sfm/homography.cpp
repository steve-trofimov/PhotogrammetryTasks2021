...
//
        ...
        const int n_trials = static_cast<int>(log(0.01) / log(1 - pow(0.5, n_samples))); // p = 0.99, w = 0.5
...
{
    cv::Vec3d hom_point{pt.x, pt.y, 1};

    cv::Mat result = T * hom_point;

    if (result.at<double>(2) == 0)
        throw std::runtime_error("W equals zero");

    return {result.at<double>(0) / result.at<double>(2), result.at<double>(1) / result.at<double>(2)};
}
