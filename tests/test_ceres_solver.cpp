...
TEST (CeresSolver, HelloWorld1) {
    ...
    // как я понял из документации, за это отвечают строчки:
    //if (jacobians != NULL && jacobians[0] != NULL) {
    //    jacobians[0][0] = -1;
    //}
    // Проверка на то, является ли jacobian = NULL.
    // Это происходит из-за того, что нам хватит простой производной из-за размерности.
...
public:
    ...
    bool operator()(const T* const queryPoint, T* residual) const {
        ...
        T dz = queryPoint[2] - center[2];
        residual[0] = a*dx*dx + b*dy*dy - dz;
        return true;
    }
protected:
    double center[3];
...
TEST (CeresSolver, HelloWorld2) {
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // TODO 3 -- done: раскомментируйте^, почему он находит не то что ожидалось?
        // Неправильно находилась невязка до параболы, так как не учитывала расстояние по оси z.
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    // Пост-обработка - приведем прямые к такой форме: x + b/a * y + c/ a = 0
    double new_ideal_line[3];
    new_ideal_line[0] = 1.;
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // Расхождение происходит из-за того, что наша прямая записывается в виде ax+by+c = 0. Неоднозначность возникает из-за того, что при домножении на константу
        // суть прямой не измениться, но при этом коэффициенты будут другими.
        // При исправлении постпроцессингом можно привести прямые к одному виду, например : x + b/a * y + c/ a. Другой вариант - изначально искать меньшее количество
        // параметров, в том же виде : x + a * y + b = 0
        // TODO 7 -- done поправьте тест так или иначе (хотя бы пост-процессингом)
        // поправил на пост-процессинг
    }
